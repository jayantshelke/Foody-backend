...
const HomePage = () => {
  const { cart, addToCart, removeFromCart, deleteFromCart, getCartTotal, getTotalItems } = useCart();
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6 bg-[url('https://e1.pxfuel.com/desktop-wallpaper/511/370/desktop-wallpaper-design-food-backgrounds-black-food.jpg')] bg-fixed bg-no-repeat bg-cover">
      ...
      <Link to="/" className="underline mb-4 inline-block">
        <button className='bg-none shadow-lg button rounded-md p-2'>Go to home</button>
      </Link>
          ...
          {cart.map((item) => (
            <li key={item.idMeal} className="bg-white bg-opacity-25 rounded-lg shadow-lg p-4 flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                {/* Show trash icon if quantity is 1, else show the minus button */}
                {item.quantity === 1 ? (
                  <button
                    onClick={() => deleteFromCart(item.idMeal)}
                    className="bg-slate-600 hover:text-red-500 text-white py-2 px-4 rounded-lg shadow-lg"
                  >
